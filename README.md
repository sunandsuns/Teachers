...
账号与个人数据（上面那八页同样要登录；这三页是"账号"本身，其中登录页就是入口）：
...
> **打开网站第一眼就是登录页**：登录之前一个页面也看不了，`/` 与 `/books/:id`
> 也不例外。
>
> 门开在哪儿是产品判断：这套东西的每一页都长在"你的记录"上——书架收藏、求教
> 记录、归纳出的画像。没有账号时它们要么是空的，要么无处可归属。与其让第一次
> 来的人先看到一个空壳再被逐页拦住，不如开门见山：先注册，再进。
>
> 被拦下时**跳到登录页**（`/login`），登完自动回到原来那一页。来路记在
> `state.from` 上，中途转去注册页也带着——否则用户刚点过的入口会被悄悄忘掉。
> 登录页还会据此点一句名——表单上方写着「登录后回到『寻章』」，让用户知道自己那一下没白点。
> 从首页（`/`）来则不点名：那里本来是默认落点，说"回到首页"是废话。
>
> 登录页是**独立整屏**（`AuthLayout`，与正式界面壳 `AppShell` 并列），
> **不显示顶栏导航**：那排入口在还没登录的人眼里是七个同样的死胡同，
> 点哪个都会回到这一页。留下的只有"这是什么产品"与"怎么进去"两件事。
>
> 登录换来的是"你的记录归你"——「回响」与「画像」在登录后按账号分开存，
> `user_id IS NULL` 是账号体系上线前**匿名访客那一份**的历史数据，两者互不可见。
> 注意它的含义是**"匿名那一份"**，不是"不过滤"：若把 `None` 当成"全部"，
> 任何一次读取都会把所有人的问答端出来。
>
> **前后端一起收紧**，否则从接口直接打进来就绕过了界面那道门：各路由在
> `APIRouter` 上挂 `dependencies=[Depends(require_user)]`（`/api/admin/*` 挂
> `require_admin`）。留开的口子只有两个——`/api/auth/*`（得能登录）与
> `/api/health`（探活）。为什么挂路由级而不是每个 handler 各写一遍，见
> `server/deps.py`：漏一处的代价是**安静地对匿名开放**，不报错。
...
> **登录之前一个接口也不放行**，下面所有接口都是这样——包括 `/api/books/*`。
...
> 反过来，其余接口**必须**给 401——那里 401 是准确答案，前端靠它把请求与登录门分开。
> 这两个口子（`/api/auth/*` 与 `/api/health`）是**仅有的**两处匿名可达的地方。
...
> 不先登，后面每一段都会齐刷刷拿到 401，而报错长得像"后端坏了"。而且 **cookie 按 host 分**，
> 所以 `localhost:5173` 与 `127.0.0.1:8000` 各登一次——否则 A 段那个"直连后端"的对照就没有身份。
> L 段里另有一个**匿名会话**，专门逐条断言那 11 个受保护接口都回 401、而 `/api/health` 仍放行。

`ui_check.py` 打开真实 Chrome，等数据落定后截图，并断言页面**看得见**：
**打开首页就落在登录页（地址栏 `/login`、有邮箱框、没有顶栏导航、看不到书架内容）**、
**匿名逐个走 10 条路由（含 `/` 与 `/books/01`）都被送到登录页**、从被拦下的那一页
登完回到**原来那一页**（断言的是浏览器地址栏，不是"页面上出现了某个词"）、
**登录之后同一批页面真的渲染出来了**（15 张书卡都入场、我的书架有检索框——
这是前两段的反面对照，否则整站坏在登录页上时那两段照样全绿）、错误密码会报错、
...
| 前端 | **`route-access.test.tsx`** | **访问边界**：渲染真实的 `<App />`（**不是**在测试里抄一份路由表），逐个断言 `GATED_PATHS`（含 `/` 与 `/books/01`）在登录之前**全部**被送到登录页、被拦时连数据都不去取，以及"登完回到原页""来路点名""从首页进来就不点名"与"中途转注册页来路不丢" |
...
**匿名访客那一份**（账号体系上线前留下的历史数据；现在没有接口能在未登录时读到它，
但语义不能变）。若把 `None` 读成"全部"，任何一次读取都会把所有人的问答端出来，
而这正是加用户系统要防的事。所以"这份记录属于谁"被写成一个显式的 WHERE 片段，
并**与外层条件用 `AND` 拼在同一条 SQL 里**：
...
界面上**登录之前一个页面也看不了**（`/` 与 `/books/:id` 也不例外，见「功能」那一节的说明）。
登录这道门**不上移到路由之外**：它是一条无路径的布局路由，因此"哪些页面要登录"
只有路由表一个答案——漏包一条不会报错，只会安静地对匿名开放，让清单能被
测试（`route-access.test.tsx`）、冒烟（`smoke.py` L 段的 11 条 401）和界面检查
（`ui_check.py` 第 2 段）守住。
